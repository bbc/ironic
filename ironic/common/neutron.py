...
def _get_segment_by_subnet_uuid(client, subnet_uuid):
    ...
    :returns: A dict describing the neutron segment.
...
def get_physnets_by_port_uuid(client, port_uuid):
    ...
    is assigned to a subnet, return the first physnet associated with its
    segment instead.
    ...
    if network.segments is not None:
        # If a port already has IP addresses assigned, this should indicate
        ...
        for fixed_ip in port.get('fixed_ips', []):
            ...
            # Return the first valid physnet found
            if physnet:
                return set([physnet])
